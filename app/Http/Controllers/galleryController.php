...
class galleryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        Image::make($image)->resize(400, 400, function ($constraint) {  //iki nggo tamnel
    ...
    public function destroy($id)
    {
        ...
        // $gallery->delete();
        // return redirect()->back();
        if (File::exists(public_path('img/gallery/' . $gallery->image))) {
            File::delete(public_path('img/gallery/' . $gallery->image));
            File::delete(public_path('img/gallery-original/' . $gallery->image));
            $gallery->delete();
